...
def extract_all_landmarks(landmarks):
    """Extract all landmark positions from pose landmarks"""
    extracted_landmarks = []
    
    try:
        for i, landmark in enumerate(landmarks):
            if landmark.get('visibility', 0) > 0.5:  # Only include visible landmarks
...
def transform_landmarks_to_svg_coordinates(landmarks, calibration_utils, transform_matrix, image_size=(1280, 720)):
    """Transform normalized landmark positions to SVG coordinates"""
    transformed_landmarks = []
    
    for landmark in landmarks:
        # Convert normalized position to image coordinates
        img_x = landmark['x'] * image_size[0]
        img_y = landmark['y'] * image_size[1]
...
class WebSocketPoseTransformer:
    ...
    async def handle_pose_data(self, pose_data):
        ...
        try:
            ...
            extracted_landmarks = extract_all_landmarks(landmarks)
            
            # Transform coordinates
            transformed_landmarks = transform_landmarks_to_svg_coordinates(
                extracted_landmarks, self.calibration_utils, self.transform_matrix
            )
...
class Command(BaseCommand):
    help = 'WebSocket-based pose coordinate transformer for climbing walls'
